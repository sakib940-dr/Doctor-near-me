...
const copy={
  bn:{back:'হাসপাতাল/চেম্বার তালিকায় ফিরুন',hospital:'হাসপাতাল',chamber:'চেম্বার',call:'কল করুন',whatsapp:'WhatsApp',contact:'অ্যাপয়েন্টমেন্ট/যোগাযোগ',save:'সংরক্ষণ',direction:'দিকনির্দেশনা',followers:'মোট অনুসারী',reviews:'রিভিউ',rating:'গড় রেটিং',about:'হাসপাতাল সম্পর্কে',hours:'খোলার সময়',services:'সেবাসমূহ',treatment:'চিকিৎসা/সেবা খরচ',investigation:'পরীক্ষা/ইনভেস্টিগেশন খরচ',doctors:'এই হাসপাতালের ডাক্তারগণ',allDoctors:'সব দেখুন',map:'যোগাযোগ, ম্যাপ ও দূরত্ব',distance:'আপনার অবস্থান থেকে',showDistance:'আমার দূরত্ব দেখুন',locating:'Location নেওয়া হচ্ছে…',open:'এখন খোলা',closed:'এখন বন্ধ',hours24:'২৪ ঘণ্টা খোলা',noInfo:'তথ্য এখনো যোগ করা হয়নি।',noDoctors:'Reception এখনো কোনো Doctor card প্রকাশ করেনি।',costDisclaimer:'খরচ সেবা, প্যাকেজ ও রোগীর প্রয়োজন অনুযায়ী পরিবর্তিত হতে পারে।',premium:'Premium',verified:'Verified',new:'নতুন',unverified:'যাচাই হয়নি',commonContact:'Hospital appointment contact'},
  en:{back:'Back to hospitals',hospital:'Hospital',chamber:'Chamber',call:'Call Now',whatsapp:'WhatsApp',contact:'Appointment / Contact',save:'Save',direction:'Directions',followers:'Followers',reviews:'Reviews',rating:'Average rating',about:'About Hospital',hours:'Opening Hours',services:'Services',treatment:'Treatment / Service Costs',investigation:'Investigation Costs',doctors:'Doctors at this hospital',allDoctors:'View all',map:'Contact, Map & Distance',distance:'From your location',showDistance:'Show my distance',locating:'Getting location…',open:'Open now',closed:'Closed now',hours24:'Open 24 hours',noInfo:'Information has not been added yet.',noDoctors:'Reception has not published a Doctor card yet.',costDisclaimer:'Costs may vary by service, package and patient requirements.',premium:'Premium',verified:'Verified',new:'New',unverified:'Unverified',commonContact:'Hospital appointment contact'},
} as const;
...
export default function PublicProviderProfilePage(){
  ...
  const [loading,setLoading]=useState(isSupabaseConfigured),[error,setError]=useState<string|null>(null),[activeSlide,setActiveSlide]=useState(0),[contactOpen,setContactOpen]=useState(false),[distance,setDistance]=useState<number|null>(null),[distanceBusy,setDistanceBusy]=useState(false),[distanceError,setDistanceError]=useState<string|null>(null),[doctorLimit,setDoctorLimit]=useState(20);
  const sliderRef=useRef<HTMLDivElement>(null),trackedView=useRef<string|null>(null);
  useEffect(()=>setDoctorLimit(20),[providerId]);
  ...
  useEffect(()=>{if(provider?.provider_type!=='hospital'||sliderImages.length<2)return;const timer=window.setInterval(()=>{if(document.hidden)return;const el=sliderRef.current;if(!el)return;const width=el.clientWidth||1;const current=Math.round(el.scrollLeft/width);el.scrollTo({left:((current+1)%sliderImages.length)*width,behavior:'smooth'});},4500);return()=>window.clearInterval(timer)},[provider?.provider_type,sliderImages.length]);
  ...
  return <div className="app-shell provider-public-v2"><PublicHeader mobileBottomNav/><main className="container public-provider-v2-main">
    ...
    <section className={`provider-slider-v2${provider.provider_type==='hospital'?' hospital-slider-v2':''}`}>
      {sliderImages.length?<><div className="provider-slider-track-v2" ref={sliderRef} onScroll={e=>{const el=e.currentTarget;const width=el.clientWidth||1;setActiveSlide(Math.max(0,Math.min(sliderImages.length-1,Math.round(el.scrollLeft/width))))}}>{sliderImages.map((img,index)=><figure key={img.id}><img src={img.src} alt={img.caption||provider.name_bn} loading={index===0?'eager':'lazy'} decoding="async" width="1600" height="900" onError={(event)=>{event.currentTarget.style.display='none'}}/>{img.caption&&<figcaption>{img.caption}</figcaption>}</figure>)}</div>{sliderImages.length>1&&<div className="provider-slider-dots-v2">{sliderImages.map((x,i)=><button type="button" key={x.id} className={i===activeSlide?'active':''} onClick={()=>scrollSlide(i)} aria-label={`Slide ${i+1}`}/>)}</div>}</>:<div className="provider-slider-fallback-v2">{provider.logo_url?<img src={getImageUrl(provider.logo_url,'public-images','thumbnail')||provider.logo_url} alt={provider.name_bn}/>:<Building2/>}</div>}
    </section>

    <section className="provider-summary-card-v2">
      <div className="provider-summary-logo-v2">{provider.logo_url?<img src={getImageUrl(provider.logo_url,'public-images','thumbnail')||provider.logo_url} alt=""/>:<Building2/>}</div>
    ...
    <section className="provider-doctors-v2"><div className="visitor-section-head"><div><span>{typeLabel}</span><h2>{t.doctors}</h2></div><b>{managedDoctors.length}</b></div>{managedDoctors.length?<><div className="hospital-public-doctor-list">{managedDoctors.slice(0,doctorLimit).map(doctor=><HospitalDoctorCard key={doctor.id} doctor={doctor} hospital={provider}/>)}</div>{doctorLimit<managedDoctors.length&&<button className="hospital-doctor-show-more" type="button" onClick={()=>setDoctorLimit(limit=>limit+20)}>{language==='bn'?'আরও ২০ জন দেখুন':'Show 20 more'} <span>{Math.min(doctorLimit,managedDoctors.length)}/{managedDoctors.length}</span></button>}</>:<div className="visitor-empty">{t.noDoctors}</div>}</section>
