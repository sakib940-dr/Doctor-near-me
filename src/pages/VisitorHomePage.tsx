  ...
  return (
    <div className="app-shell visitor-home">
      <header className="visitor-header">
        <div className="container visitor-header-inner">
          <Link className="visitor-brand" to="/"><span><HeartPulse /></span><strong>{siteName}</strong></Link>
          <nav className={menuOpen ? 'visitor-nav open' : 'visitor-nav'}>
            <Link to="/doctors" onClick={() => setMenuOpen(false)}>ডাক্তার</Link>
            <Link to="/providers" onClick={() => setMenuOpen(false)}>হাসপাতাল/চেম্বার</Link>
            <a href="#blood" onClick={() => setMenuOpen(false)}>ব্লাড ব্যাংক</a>
            <a href="#ambulance" onClick={() => setMenuOpen(false)}>অ্যাম্বুলেন্স</a>
            <Link className="visitor-login" to="/auth"><UserRound /> লগইন</Link>
          </nav>
          <button className="visitor-menu" type="button" onClick={() => setMenuOpen((value) => !value)}>{menuOpen ? <X /> : <Menu />}</button>
        </div>
      </header>
      ...
      <main>
        <section className="visitor-search-hero">
          <div className="container">
            <div className="visitor-hero-copy">
              <span><BadgeCheck /> যাচাইকৃত স্বাস্থ্যসেবা</span>
              <h1>আপনার কাছের সঠিক ডাক্তার খুঁজুন</h1>
              <p>জেলা, উপজেলা, ডাক্তার বা বিশেষত্ব দিয়ে দ্রুত খুঁজুন।</p>
            </div>
            <form className="visitor-search-box" onSubmit={submitSearch}>
              ...
              <label className="visitor-search-input"><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ডাক্তার, রোগ বা স্পেশালিটি" /></label>
        ...
        <section className="visitor-section">
          <div className="container">
            <div className="visitor-section-head"><div><span>কাছাকাছি সেবা</span><h2>আপনার এলাকার ডাক্তার</h2></div><Link to={`/doctors${viewAllParams.size ? `?${viewAllParams}` : ''}`}>সব দেখুন <ArrowRight /></Link></div>
        ...
        <section className="visitor-section soft-section">
          <div className="container">
            <div className="visitor-section-head"><div><span>ডাক্তার খুঁজুন</span><h2>জনপ্রিয় / সকল ডাক্তার</h2></div><Link to="/doctors">সব দেখুন <ArrowRight /></Link></div>
        ...
        <section className="visitor-section">
          <div className="container">
            <div className="visitor-section-head"><div><span>বিশেষজ্ঞ বিভাগ</span><h2>Speciality categories</h2></div><Link to="/doctors">সব দেখুন <ArrowRight /></Link></div>
        ...
        <section className="visitor-section soft-section" id="hospitals">
          <div className="container">
            <div className="visitor-section-head"><div><span>চিকিৎসা প্রতিষ্ঠান</span><h2>Hospital / Chamber</h2></div><Link to="/providers">সব দেখুন <ArrowRight /></Link></div>
