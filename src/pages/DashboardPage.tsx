...
import { Ambulance, Bell, Building2, CalendarDays, ChevronRight, Clock3, Crown, Droplets, FileCheck2, LoaderCircle, LogOut, MapPin, Settings, ShieldCheck, Stethoscope, UserRound, Activity, CalendarClock, Search, CheckCircle2 } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const cards = role === 'patient'
    ...
    : role === 'doctor'
      ? [{ icon: Activity, title: 'Profile Analytics', detail: 'Views, clicks, follower ও review metrics', path: '/doctor/analytics' }, { icon: Stethoscope, title: 'ডাক্তার প্রোফাইল', detail: context?.doctor?.verification_status === 'approved' ? 'প্রোফাইল অনুমোদিত' : 'Verification অপেক্ষমাণ', path: '/doctor/profile' }, { icon: FileCheck2, title: 'Verification evidence', detail: 'BMDC, degree ও identity evidence', path: '/verification/evidence' }, { icon: MapPin, title: 'Location ও Public Map', detail: 'GPS দিয়ে Latitude/Longitude ও chamber map যোগ করুন', path: '/doctor/chambers' }, { icon: Clock3, title: 'চেম্বার ও সময়সূচি', detail: 'নিজস্ব visiting day, সময় ও fee পরিচালনা করুন', path: '/doctor/schedules' }, { icon: CalendarDays, title: 'অ্যাপয়েন্টমেন্ট', detail: 'রোগীর request, serial ও status পরিচালনা করুন', path: '/doctor/appointments' }]
      : role === 'hospital' || role === 'chamber'
        ? [{ icon: Activity, title: 'Profile Analytics', detail: 'Views, calls, appointments ও follower metrics', path: '/provider/analytics' }, { icon: Building2, title: 'প্রতিষ্ঠানের প্রোফাইল', detail: `${context?.providers?.length ?? 0}টি প্রতিষ্ঠান যুক্ত`, path: '/provider/profile' }, { icon: FileCheck2, title: 'Verification evidence', detail: 'Trade license ও organization evidence', path: '/verification/evidence' }, { icon: Stethoscope, title: 'Reception Doctor Cards', detail: 'Independent Doctor card ও reception contact', path: '/provider/doctors' }, { icon: CalendarDays, title: 'Reception appointments', detail: 'Serial request ও appointment queue', path: '/provider/appointments' }, ...(role === 'hospital' ? [{ icon: Ambulance, title: 'Ambulance links', detail: 'সংযোগ request approve বা reject করুন', path: '/provider/ambulances' }] : [])]
        : role === 'ambulance'
