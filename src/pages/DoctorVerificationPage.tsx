            ...
            <section className="doctor-verification-evidence-card">
              ...
              {evidenceEditable ? (
                <form className="evidence-upload doctor-verification-upload" onSubmit={upload}>
                  ...
                  <label><FilePlus2 /> {file?.name || 'ফাইল নির্বাচন'}<input type="file" required accept="image/jpeg,image/png,image/webp,application/pdf" onChange={(event: ChangeEvent<HTMLInputElement>) => setFile(event.target.files?.[0] || null)} /></label>
