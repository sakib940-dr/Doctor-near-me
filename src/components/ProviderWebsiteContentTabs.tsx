...
export default function ProviderWebsiteContentTabs({providerId,initialTab='services',hideNavigation=false,costMode='both'}:{providerId:string;initialTab?:ProviderWebsiteTab;hideNavigation?:boolean;costMode?:ProviderCostMode}){const [tab,setTab]=useState<ProviderWebsiteTab>(initialTab);useEffect(()=>setTab(initialTab),[initialTab]);const labels:Record<ProviderWebsiteTab,string>={services:'Services',hours:'Opening Hours',slider:'Public Slider',gallery:'Gallery',costs:'Treatment / Investigation Costs',reviews:'Reviews'};return <div className="provider-web-admin">{!hideNavigation&&<nav className="provider-web-tabs">{(['services','hours','slider','costs','gallery','reviews'] as ProviderWebsiteTab[]).map(t=><button type="button" className={tab===t?'active':''} onClick={()=>setTab(t)} key={t}>{labels[t]}</button>)}</nav>}{tab==='services'&&<Services providerId={providerId}/>} {tab==='hours'&&<OpeningHours providerId={providerId}/>} {tab==='slider'&&<Slider providerId={providerId}/>} {tab==='gallery'&&<Gallery providerId={providerId}/>} {tab==='costs'&&<Costs providerId={providerId} mode={costMode}/>} {tab==='reviews'&&<Reviews providerId={providerId}/>}</div>}
