...
const messageFrom = (error: unknown) => {
  ...
  if (/row-level security|violates.*policy|unauthorized/i.test(message)) return 'Upload permission পাওয়া যায়নি। সর্বশেষ SQL migration deploy করে আবার চেষ্টা করুন।';
  if (/payload|too large|maximum|size/i.test(message)) return 'File size অনুমোদিত সীমার বেশি। ছোট file দিয়ে চেষ্টা করুন।';
  return message || 'Onboarding তথ্য সংরক্ষণ করা যায়নি।';
};
...
function DoctorVerificationStep({ onError, onNext, onPrevious }: StepProps) {
  ...
  const [uploadError,setUploadError]=useState<string|null>(null); const [uploadNotice,setUploadNotice]=useState<string|null>(null);
  useUnsavedWarning(dirty);
  ...
  async function loadEvidence(){if(user)setEvidence(await getMyEntityVerificationEvidence('doctor',user.id));}
  useEffect(()=>{load().catch(e=>onError(messageFrom(e)));},[user?.id, onError]);
  async function remove(doc:VerificationEvidenceDocument){if(locked)return;setSaving(true);setUploadError(null);try{await deleteEntityVerificationDocument(doc.document_id);await loadEvidence();setUploadNotice('Document মুছে ফেলা হয়েছে।');}catch(e){const message=messageFrom(e);setUploadError(message);onError(message);}finally{setSaving(false);}}
  async function submit(e:FormEvent){
    e.preventDefault();onError(null);setUploadError(null);setUploadNotice(null);
    setSaving(true);
    try{
      if(!locked){
        if(!medicalType) throw new Error('Medical Type নির্বাচন করুন।');
        await updateMyDoctorVerificationInfo({medicalType:medicalType as MedicalType,medicalCollege:college,medicalSession:session,medicalBatch:batch,bmdcRegistrationNo:bmdc});
        let hasEvidence=Boolean(evidence?.documents.length);
        if(file&&user){
          await uploadEntityVerificationDocument({entityType:'doctor',entityId:user.id,documentType:docType,file});
          setFile(null);
          await loadEvidence();
          hasEvidence=true;
        }
        if(hasEvidence)await submitMyDoctorVerificationApplication();
      }
      setDirty(false);await onNext();
    }catch(err){const message=messageFrom(err);setUploadError(message);onError(message);}finally{setSaving(false);}
  }
  return <form className="onboarding-card professional-step-card" onSubmit={submit} onChange={()=>setDirty(true)}><header><FileCheck2/><div><small>Step 2</small><h2>Verification Form</h2><p>Medical College, Session, Batch ও BMDC Number পূরণ করুন। Document এখন না দিলে Save & Next করে পরের step-এ যেতে পারবেন এবং পরে dashboard থেকে যোগ করতে পারবেন।</p></div></header>{locked&&<div className="identity-status verified"><ShieldCheck/><span><strong>{verificationStatus==='approved'?'Verification approved':'Application pending'}</strong><small>Submitted verification identity locked।</small></span></div>}{verificationStatus==='rejected' && !locked && <div className="auth-message error"><strong>Rejected:</strong> পুনরায় তথ্য/document ঠিক করে onboarding continue করলে Re-Verification submit হবে।</div>}<div className="patient-form-grid"><label className="auth-field"><span>Medical Type</span><div><Stethoscope/><input readOnly value={medicalType}/></div></label><label className="auth-field"><span>Medical College Name *</span><div><GraduationCap/><input required minLength={2} disabled={locked} value={college} onChange={e=>setCollege(e.target.value)}/></div></label><label className="auth-field"><span>Session *</span><div><input required disabled={locked} value={session} onChange={e=>setSession(e.target.value)} placeholder="e.g. 2015-2016"/></div></label><label className="auth-field"><span>Batch *</span><div><input required disabled={locked} value={batch} onChange={e=>setBatch(e.target.value)} placeholder="e.g. 25th Batch"/></div></label><label className="auth-field"><span>BMDC Registration Number *</span><div><ShieldCheck/><input required minLength={3} disabled={locked} value={bmdc} onChange={e=>setBmdc(e.target.value)} /></div></label></div><EvidenceEditor evidence={evidence} file={file} setFile={setFile} documentType={docType} setDocumentType={setDocType} onDelete={remove} doctor deferredUpload disabled={locked} busy={saving} operationError={uploadError} operationNotice={uploadNotice}/><StepActions onPrevious={onPrevious} saving={saving} label={locked?'Continue':'Save & Next'}/></form>;
}

function DoctorVisitingStep({ onError, onNext, onPrevious }: StepProps) {
  const { user } = useAuth();
  const [profile, setProfile] = useState<MyDoctorProfile | null>(null); const [specialties, setSpecialties] = useState<Specialty[]>([]); const [photo, setPhoto] = useState<File | null>(null); const [photoPreview,setPhotoPreview]=useState<string|null>(null); const [removePhoto,setRemovePhoto]=useState(false); const [saving, setSaving] = useState(false); const [dirty,setDirty]=useState(false);
  useUnsavedWarning(dirty);
...
function DoctorServicesStep({onError,onNext,onPrevious}:StepProps){
  ...
  const [localError,setLocalError]=useState<string|null>(null);const [localNotice,setLocalNotice]=useState<string|null>(null);
  async function load(){setContent(await getMyDoctorPublicContent());}
  useEffect(()=>{load().catch(e=>onError(messageFrom(e)));},[onError]);
  async function add(){if(!name.trim())return;setSaving(true);onError(null);setLocalError(null);setLocalNotice(null);try{const created=await doctorServices.create({name:{bn:name.trim()},description:description.trim()?{bn:description.trim()}: {},is_active:true,sort_order:(content?.services.length||0)+1});setContent(current=>current?{...current,services:[...(current.services||[]),created]}:current);setName('');setDescription('');setLocalNotice('Service যোগ হয়েছে এবং নিচের তালিকায় দেখানো হচ্ছে।');}catch(e){const message=messageFrom(e);setLocalError(message);onError(message);}finally{setSaving(false);}}
  async function remove(id:number){setSaving(true);setLocalError(null);try{await doctorServices.remove(id);setContent(current=>current?{...current,services:current.services.filter(item=>item.id!==id)}:current);}catch(e){const message=messageFrom(e);setLocalError(message);onError(message);}finally{setSaving(false);}}
  return <section className="onboarding-card professional-step-card"><header><Stethoscope/><div><small>Step 6</small><h2>Service List</h2><p>Existing service management system reuse হচ্ছে। Service না দিয়েও continue করা যাবে।</p></div></header><div className="onboarding-public-note"><ShieldCheck/><span><strong>Public & Optional</strong><small>Later Public Content Management থেকে add/edit/delete করা যাবে।</small></span></div><div className="onboarding-inline-editor"><input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Dental Consultation"/><input value={description} onChange={e=>setDescription(e.target.value)} placeholder="Short description (optional)"/><button type="button" className="secondary-action" disabled={saving||!name.trim()} onClick={()=>void add()}>{saving?<LoaderCircle className="spin"/>:<Save/>} Add Service</button></div>{localError&&<div className="doctor-content-inline-error" role="alert">{localError}</div>}{localNotice&&<div className="doctor-content-inline-success" role="status">{localNotice}</div>}<div className="onboarding-simple-list">{content?.services?.length?content.services.map(item=><div key={item.id}><span><strong>{item.name.bn||item.name.en||'Service'}</strong><small>{item.description?.bn||item.description?.en||''}</small></span><button type="button" disabled={saving} onClick={()=>void remove(item.id)} aria-label="Delete service"><Trash2/></button></div>):<p>কোনো service যোগ করা হয়নি। চাইলে Skip করুন।</p>}</div><div className="onboarding-step-actions"><button type="button" className="secondary-action" onClick={onPrevious}><ArrowLeft/> Back</button><button type="button" className="secondary-action onboarding-skip-action" disabled={saving} onClick={()=>void onNext()}>Skip</button><button type="button" className="auth-submit" disabled={saving} onClick={()=>void onNext()}>Save & Continue <ArrowRight/></button></div></section>;
}
...
function EvidenceEditor({ evidence,file,setFile,documentType,setDocumentType,onUpload,onDelete,doctor=false,deferredUpload=false,disabled=false,busy=false,operationError=null,operationNotice=null }:{evidence:OwnerVerificationEvidence|null;file:File|null;setFile:(f:File|null)=>void;documentType:string;setDocumentType:(v:string)=>void;onUpload?:()=>Promise<void>;onDelete:(d:VerificationEvidenceDocument)=>Promise<void>;doctor?:boolean;deferredUpload?:boolean;disabled?:boolean;busy?:boolean;operationError?:string|null;operationNotice?:string|null}) {
  ...
  return <section className="onboarding-evidence">
    <header><FilePlus2/><div><h3>Verification evidence</h3><p>{deferredUpload?'Document থাকলে নির্বাচন করুন; না থাকলে Save & Next করুন।':'Documents private verification-documents bucket-এ থাকবে।'}</p></div></header>
    ...
    {!deferredUpload&&onUpload&&<button type="button" className="secondary-action evidence-upload-button" disabled={disabled||busy||!file} onClick={()=>void onUpload()}>{busy?<LoaderCircle className="spin"/>:<FilePlus2/>} {busy?'Uploading…':'Upload'}</button>}
    <div className="onboarding-evidence-list">{evidence?.documents.length ? evidence.documents.map(doc=><div key={doc.document_id}><FileCheck2/><span><strong>{doc.document_type.replaceAll('_',' ')}</strong><small>{formatDateSafe(doc.created_at, 'bn-BD', { dateStyle: 'medium' }, 'তারিখ নেই')}</small></span><button type="button" disabled={disabled||busy} onClick={()=>void onDelete(doc)} aria-label="Delete document"><Trash2/></button></div>):!file?<p>Document পরে dashboard থেকেও যোগ করা যাবে।</p>:null}</div>
  </section>;
}
