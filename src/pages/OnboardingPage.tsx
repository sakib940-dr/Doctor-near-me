...
function DoctorVerificationStep({ onError, onNext, onPrevious }: StepProps) {
  ...
  async function upload(){if(!user||!file||locked)return;setSaving(true);onError(null);setUploadError(null);setUploadNotice(null);try{await uploadEntityVerificationDocument({entityType:'doctor',entityId:user.id,documentType:docType,file});setFile(null);await loadEvidence();setUploadNotice('Document সফলভাবে upload হয়েছে। উপরের লেখা অপরিবর্তিত আছে।');}catch(e){const message=messageFrom(e);setUploadError(message);onError(message);}finally{setSaving(false);}}
...
function EvidenceEditor({ evidence,file,setFile,documentType,setDocumentType,onUpload,onDelete,doctor=false,disabled=false,busy=false,operationError=null,operationNotice=null }:{evidence:OwnerVerificationEvidence|null;file:File|null;setFile:(f:File|null)=>void;documentType:string;setDocumentType:(v:string)=>void;onUpload:()=>Promise<void>;onDelete:(d:VerificationEvidenceDocument)=>Promise<void>;doctor?:boolean;disabled?:boolean;busy?:boolean;operationError?:string|null;operationNotice?:string|null}) {
  ...
  return <section className="onboarding-evidence"><header><FilePlus2/><div><h3>Verification evidence</h3><p>Documents private verification-documents bucket-এ থাকবে।</p></div></header><div className="evidence-upload-row"><select disabled={disabled||busy} value={documentType} onChange={e=>setDocumentType(e.target.value)}>{options.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select><input key={file?`${file.name}-${file.lastModified}`:'empty-file'} disabled={disabled||busy} type="file" data-skip-global-guard="true" accept="image/jpeg,image/png,image/webp,image/avif,application/pdf" onChange={handleFileChange}/>{(error||operationError)&&<small className="form-error" role="alert">{error||operationError}</small>}{operationNotice&&<small className="form-success" role="status">{operationNotice}</small>}{previewUrl&&<img className="evidence-preview" src={previewUrl} alt="Selected document preview"/>}<small className="image-upload-hint">ছবির প্রস্তাবিত সর্বোচ্চ 2200×2200 px • Original image সর্বোচ্চ 5 MB • Upload-এর আগে 100–200 KB WebP-তে auto-compress হবে • PDF অপরিবর্তিত</small><button type="button" className="secondary-action" disabled={disabled||busy||!file} onClick={()=>void onUpload()}>{busy?<LoaderCircle className="spin"/>:<FilePlus2/>} {busy?'Uploading…':'Upload'}</button></div><div className="onboarding-evidence-list">{evidence?.documents.length ? evidence.documents.map(doc=><div key={doc.document_id}><FileCheck2/><span><strong>{doc.document_type.replaceAll('_',' ')}</strong><small>{formatDateSafe(doc.created_at, 'bn-BD', { dateStyle: 'medium' }, 'তারিখ নেই')}</small></span><button type="button" disabled={disabled||busy} onClick={()=>void onDelete(doc)} aria-label="Delete document"><Trash2/></button></div>):<p>Evidence এখনো upload করা হয়নি। চাইলে পরে dashboard থেকেও যোগ করতে পারবেন।</p>}</div></section>;
}
