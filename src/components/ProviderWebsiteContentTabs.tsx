...
const msg=(e:unknown)=>e instanceof Error?e.message:'কাজটি সম্পন্ন করা যায়নি';
...
function Services({providerId}:{providerId:string}){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    e.preventDefault();setBusy(true);setError(null);const f=new FormData(e.currentTarget);const file=f.get('imageFile');let uploaded:string|null=null;
    try{
      ...
      setEdit(null);e.currentTarget.reset();await load();
...
function Slider({providerId}:{providerId:string}){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    setBusy(true);setError(null);const f=new FormData(e.currentTarget);
    try{
...
function Reviews({providerId}:{providerId:string}){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    try{if(edit)await providerReviews.update(providerId,edit.id,input);else await providerReviews.create(providerId,input);setEdit(null);e.currentTarget.reset();await load();}catch(e2){setError(msg(e2))}
  }
