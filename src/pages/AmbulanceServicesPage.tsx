...
export default function AmbulanceServicesPage() {
  ...
  useEffect(() => { void load(); }, []);
  useEffect(() => { if (!form.districtId) { setUpazilas([]); return; } getUpazilas(form.districtId).then(setUpazilas).catch(() => setError('উপজেলা / এলাকার তালিকা লোড করা যায়নি।')); }, [form.districtId]);
