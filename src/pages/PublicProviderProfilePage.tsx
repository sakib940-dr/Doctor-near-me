...
const localText=(value:{bn?:string|null;en?:string|null}|null|undefined,language:Language)=>(language==='bn'?(value?.bn||value?.en):(value?.en||value?.bn))||'';
const numberText=(value:number,language:Language,digits=0)=>value.toLocaleString(language==='bn'?'bn-BD':'en-US',{minimumFractionDigits:digits,maximumFractionDigits:digits});
...
function rankLabel(tier:PublicRankingTier|undefined,language:Language,verified:boolean){const t=copy[language];if(tier==='premium')return t.premium;if(tier==='verified'||verified)return t.verified;if(tier==='new')return t.new;return t.unverified;}

function openingStatus(hours:ProviderOpeningHour[],language:Language,fallback:string|null|undefined){
...
function todayDoctorSchedule(doctor:DoctorSearchRow,language:Language){
...
export default function PublicProviderProfilePage(){
  ...
  const t=copy[language],typeLabel=provider?.provider_type==='hospital'?t.hospital:t.chamber,phone=provider?.phone?cleanPhone(provider.phone):null,wa=provider?.whatsapp?whatsappNumber(provider.whatsapp):(provider?.phone?whatsappNumber(provider.phone):null),status=openingStatus(content?.opening_hours||[],language,provider?.opening_note);
  ...
  return <div className="app-shell provider-public-v2"><PublicHeader mobileBottomNav/><main className="container public-provider-v2-main">
    <div className="provider-public-v2-topline"><Link className="back-link" to="/providers"><ArrowLeft/>{t.back}</Link><button className="provider-language-switch" type="button" onClick={()=>setLanguage(x=>x==='bn'?'en':'bn')}>{language==='bn'?'English':'বাংলা'}</button></div>
    ...
    <section className="provider-primary-actions-v2">
      ...
      {wa?<a href={`https://wa.me/${wa}`} target="_blank" rel="noreferrer" onClick={()=>track('whatsapp_click')}><MessageCircle/><span>{t.whatsapp}</span></a>:<button disabled><MessageCircle/><span>{t.whatsapp}</span></button>}
      <button type="button" disabled={!phone&&!wa} onClick={()=>{track('appointment_click');setContactOpen(true)}}><CalendarDays/><span>{t.contact}</span></button>
    ...
    <section className="provider-doctors-v2"><div className="visitor-section-head"><div><span>{typeLabel}</span><h2>{t.doctors}</h2></div>{doctors.length>10&&<Link to={`/providers/${provider.id}/doctors`}>{t.allDoctors} →</Link>}</div>{doctors.length?<div className="provider-doctor-rail-v2">{doctors.slice(0,10).map(doctor=><article className="provider-doctor-card-shell-v2" key={doctor.doctor_id}><DoctorResultCard doctor={doctor} stats={doctorStats[doctor.doctor_id]} onStatsChange={(id,next)=>setDoctorStats(current=>({...current,[id]:next}))}/>{todayDoctorSchedule(doctor,language)&&<div className="provider-doctor-schedule-v2"><Clock3/>{todayDoctorSchedule(doctor,language)}</div>}<div className="provider-doctor-common-contact-v2">{phone&&<a href={`tel:${phone}`} onClick={()=>track('call_click','provider_doctor_card')}><Phone/>{t.call}</a>}{wa&&<a href={`https://wa.me/${wa}`} target="_blank" rel="noreferrer" onClick={()=>track('whatsapp_click','provider_doctor_card')}><MessageCircle/>{t.whatsapp}</a>}</div></article>)}</div>:<div className="visitor-empty">{t.noDoctors}</div>}</section>

    <section className="provider-contact-map-v2"><div className="provider-section-title-v2"><MapPin/><div><small>{typeLabel}</small><h2>{t.map}</h2></div></div><div className="provider-contact-grid-v2"><div><h3>{language==='bn'?provider.name_bn:(provider.name_en||provider.name_bn)}</h3><p><MapPin/>{provider.address||t.noInfo}</p>{provider.phone&&<a href={`tel:${phone}`} onClick={()=>track('call_click')}><Phone/>{provider.phone}</a>}{provider.whatsapp&&<a href={`https://wa.me/${wa}`} target="_blank" rel="noreferrer" onClick={()=>track('whatsapp_click')}><MessageCircle/>WhatsApp</a>}{distance!=null&&<strong><Navigation/>{t.distance} {numberText(distance,language,1)} {language==='bn'?'কিমি':'km'}</strong>}</div><button className="provider-distance-button-v2" type="button" disabled={distanceBusy} onClick={()=>void captureDistance()}>{distanceBusy?<LoaderCircle className="spin"/>:<Navigation/>}{distanceBusy?t.locating:t.showDistance}</button></div>{distanceError&&<p className="provider-distance-error-v2">{distanceError}</p>}{provider.latitude!=null&&provider.longitude!=null&&<div className="provider-map-frame-v2"><iframe title={`${provider.name_bn} map`} loading="lazy" referrerPolicy="no-referrer-when-downgrade" src={`https://www.google.com/maps?q=${encodeURIComponent(`${provider.latitude},${provider.longitude}`)}&z=15&output=embed`}/></div>}{directionUrl&&<a className="provider-map-direction-v2" href={directionUrl} target="_blank" rel="noreferrer" onClick={()=>track('map_click')}><ExternalLink/>{t.direction}</a>}</section>
