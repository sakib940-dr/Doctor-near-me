...
export default function VerificationOfficerPage() {
  ...
  const load = () => { setLoading(true); setError(null); getVerificationReviewQueue(entityType === 'all' ? null : entityType, status === 'all' ? null : status).then((nextRows) => { setRows(nextRows); setSelectedReviewKeys((current) => current.filter((key) => nextRows.some((row) => `${row.entity_type}:${row.entity_id}` === key && row.status === 'pending'))); }).catch((loadError: unknown) => setError(messageFrom(loadError))).finally(() => setLoading(false)); };
  ...
  async function bulkApprove() {
    ...
    setSelectedReviewKeys([]); setBulkConfirm(false); setBulkWorking(false); load();
  }
