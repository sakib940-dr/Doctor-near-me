...
function EvidenceEditor({ evidence,file,setFile,documentType,setDocumentType,onUpload,onDelete,doctor=false,disabled=false }:{evidence:OwnerVerificationEvidence|null;file:File|null;setFile:(f:File|null)=>void;documentType:string;setDocumentType:(v:string)=>void;onUpload:()=>Promise<void>;onDelete:(d:VerificationEvidenceDocument)=>Promise<void>;doctor?:boolean;disabled?:boolean}) {
  ...
  return <section className="onboarding-evidence"><header><FilePlus2/><div><h3>Verification evidence</h3><p>Documents private verification-documents bucket-এ থাকবে।</p></div></header><div className="evidence-upload-row"><select disabled={disabled} value={documentType} onChange={e=>setDocumentType(e.target.value)}>{options.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select><input disabled={disabled} type="file" accept="image/jpeg,image/png,image/webp,image/avif,application/pdf" onChange={(e:ChangeEvent<HTMLInputElement>)=>setFile(e.target.files?.[0]||null)}/><small className="image-upload-hint">ছবি হলে প্রস্তাবিত সর্বোচ্চ 2200×2200 px • সর্বোচ্চ 3 MB • আপলোডের পর ছবি স্বয়ংক্রিয়ভাবে অপটিমাইজ হবে • PDF অপরিবর্তিত</small><button type="button" className="secondary-action" disabled={disabled||!file} onClick={()=>void onUpload()}><FilePlus2/> Upload</button></div><div className="onboarding-evidence-list">{evidence?.documents.length ? evidence.documents.map(doc=><div key={doc.document_id}><FileCheck2/><span><strong>{doc.document_type.replaceAll('_',' ')}</strong><small>{formatDateSafe(doc.created_at, 'bn-BD', { dateStyle: 'medium' }, 'তারিখ নেই')}</small></span><button type="button" disabled={disabled} onClick={()=>void onDelete(doc)} aria-label="Delete document"><Trash2/></button></div>):<p>Evidence এখনো upload করা হয়নি। চাইলে পরে dashboard থেকেও যোগ করতে পারবেন।</p>}</div></section>;
}
