...
import { deleteAdminSpecialtyImage, getAdminCmsSnapshot, saveAdminBanner, saveAdminContentPage, saveAdminPublicSetting, saveAdminSection, saveAdminSpecialty, saveAdminTopic, uploadAdminBanner, uploadAdminSpecialtyImage } from '../services/adminCms';
...
type Tab = 'specialties' | 'topics' | 'sections' | 'banners' | 'content';
...
export default function AdminCmsPage() {
  ...
  async function load() {
    ...
    try {
      const [snapshot, districtRows] = await Promise.all([getAdminCmsSnapshot(), getDistricts()]);
      setData(snapshot); setDistricts(districtRows);
  ...
  return <div className="app-shell cms-page"><main className="cms-main container"><header className="cms-heading"><span><Layers3 /></span><div><small>Audited content operations</small><h1>Reference ও Homepage CMS</h1><p>Public discovery content, ordering, visibility ও bilingual pages পরিচালনা করুন।</p></div><button onClick={() => void load()}><RefreshCw /> Refresh</button></header><nav className="cms-tabs">{([['specialties', Tags, 'Specialty'], ['topics', Search, 'Discovery topics'], ['sections', Layers3, 'Sections'], ['banners', ImagePlus, 'Banners'], ['content', FileText, 'Content & settings']] as const).map(([value, Icon, label]) => <button key={value} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}><Icon /> {label}</button>)}</nav>{error && <div className="error-box">{error}</div>}{notice && <div className="auth-message success">{notice}</div>}{loading ? <div className="loading-box"><LoaderCircle className="spin" /> CMS data লোড হচ্ছে…</div> : data && <>
