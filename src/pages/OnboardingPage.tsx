      ...
      <main className="onboarding-main container">
        {phoneLinkOnly && account?.onboarding_completed ? (
          <section className="onboarding-phone-link-shell">
            <div className="onboarding-intro"><span>Account security</span><h1>Phone login activate করুন</h1><p>এই existing account-এ verified phone identity link করলে একই password দিয়ে Email অথবা Phone—দুটো দিয়েই login করা যাবে। চাইলে এখন skip করতে পারেন।</p></div>
            ...
            <button className="onboarding-skip-link" type="button" onClick={() => navigate('/dashboard', { replace: true })}>এখন নয়, Dashboard-এ যান</button>
          </section>
        ) : <>
...
function BasicStep({ userEmail, authPhone, initialName, initialPhone, initialRole, roleLocked, initialDistrictId, initialUpazilaId, onRole, onError, onSaved }: {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (isProfessionalRole(role) && !phoneVerified) { onError('Doctor/Hospital onboarding চালিয়ে যেতে Phone Number verify করুন।'); return; }
    setWorking(true);
    try {
...
function EvidenceEditor({ evidence,file,setFile,documentType,setDocumentType,onUpload,onDelete,doctor=false }:{evidence:OwnerVerificationEvidence|null;file:File|null;setFile:(f:File|null)=>void;documentType:string;setDocumentType:(v:string)=>void;onUpload:()=>Promise<void>;onDelete:(d:VerificationEvidenceDocument)=>Promise<void>;doctor?:boolean}) {
  ...
  return <section className="onboarding-evidence"><header><FilePlus2/><div><h3>Verification evidence</h3><p>Documents private verification-documents bucket-এ থাকবে।</p></div></header><div className="evidence-upload-row"><select value={documentType} onChange={e=>setDocumentType(e.target.value)}>{options.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select><input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={(e:ChangeEvent<HTMLInputElement>)=>setFile(e.target.files?.[0]||null)}/><button type="button" className="secondary-action" disabled={!file} onClick={()=>void onUpload()}><FilePlus2/> Upload</button></div><div className="onboarding-evidence-list">{evidence?.documents.length ? evidence.documents.map(doc=><div key={doc.document_id}><FileCheck2/><span><strong>{doc.document_type.replaceAll('_',' ')}</strong><small>{new Date(doc.created_at).toLocaleDateString('bn-BD')}</small></span><button type="button" onClick={()=>void onDelete(doc)} aria-label="Delete document"><Trash2/></button></div>):<p>Evidence এখনো upload করা হয়নি। চাইলে পরে dashboard থেকেও যোগ করতে পারবেন।</p>}</div></section>;
}
