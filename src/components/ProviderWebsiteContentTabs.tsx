...
export type ProviderWebsiteTab='services'|'hours'|'slider'|'gallery'|'costs'|'reviews';
export type ProviderCostMode='both'|'treatment'|'investigation';
export default function ProviderWebsiteContentTabs({providerId,initialTab='services',hideNavigation=false,costMode='both',plainServices=false}:{providerId:string;initialTab?:ProviderWebsiteTab;hideNavigation?:boolean;costMode?:ProviderCostMode;plainServices?:boolean}){const [tab,setTab]=useState<ProviderWebsiteTab>(initialTab);useEffect(()=>setTab(initialTab),[initialTab]);const labels:Record<ProviderWebsiteTab,string>={services:'Services',hours:'Opening Hours',slider:'Public Slider',gallery:'Gallery',costs:'Treatment / Investigation Costs',reviews:'Reviews'};return <div className="provider-web-admin">{!hideNavigation&&<nav className="provider-web-tabs">{(['services','hours','slider','costs','gallery','reviews'] as ProviderWebsiteTab[]).map(t=><button type="button" className={tab===t?'active':''} onClick={()=>setTab(t)} key={t}>{labels[t]}</button>)}</nav>}{tab==='services'&&<Services providerId={providerId} plain={plainServices}/>} {tab==='hours'&&<OpeningHours providerId={providerId}/>} {tab==='slider'&&<Slider providerId={providerId}/>} {tab==='gallery'&&<Gallery providerId={providerId}/>} {tab==='costs'&&<Costs providerId={providerId} mode={costMode}/>} {tab==='reviews'&&<Reviews providerId={providerId}/>}</div>}
...
function Services({providerId,plain=false}:{providerId:string;plain?:boolean}){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    try{
      let image=plain?null:(edit?.image||null);
      if(!plain&&file instanceof File&&file.size){uploaded=await uploadProviderWebsiteImage(providerId,file,'service');image=uploaded;}
      const input={name:{bn:String(f.get('bn')||''),en:String(f.get('en')||'')},description:{bn:String(f.get('dbn')||''),en:String(f.get('den')||'')},icon:plain?null:(String(f.get('icon')||'')||null),image,is_active:f.get('active')==='on',sort_order:edit?.sort_order??rows.length};
...
function Slider({providerId}:{providerId:string}){
  ...
  return <Panel title={`Public Image Slider (${rows.length}/4)`}>
    <p className="provider-content-help">Public Hospital page-এ সর্বোচ্চ ৪টি ছবি swipe carousel হিসেবে দেখাবে। প্রস্তাবিত সাইজ: 1600×900 px • সর্বোচ্চ 5 MB • upload-এর আগে 100–200 KB WebP-তে auto-compress হবে।</p>
...
function Costs({providerId,mode}:{providerId:string;mode:ProviderCostMode}){return <div className={`provider-cost-admin-grid${mode!=='both'?' single':''}`}>{mode!=='investigation'&&<CostEditor providerId={providerId} title="Treatment Costs" api={providerTreatmentCosts}/>} {mode!=='treatment'&&<CostEditor providerId={providerId} title="Investigation Costs" api={providerInvestigationCosts}/>}</div>}
