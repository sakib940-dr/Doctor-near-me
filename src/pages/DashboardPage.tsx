...
import { Ambulance, Bell, Building2, CalendarDays, ChevronRight, LoaderCircle, LogOut, Settings, ShieldCheck, Stethoscope, UserRound } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const cards = role === 'patient'
    ...
    : role === 'doctor'
      ? [{ icon: Stethoscope, title: 'ডাক্তার প্রোফাইল', detail: context?.doctor?.verification_status === 'approved' ? 'প্রোফাইল অনুমোদিত' : 'Verification অপেক্ষমাণ', path: '#' }, { icon: CalendarDays, title: 'অ্যাপয়েন্টমেন্ট', detail: 'রোগীর request ও schedule দেখুন', path: '#' }]
      : role === 'hospital' || role === 'chamber'
        ? [{ icon: Building2, title: 'প্রতিষ্ঠানের প্রোফাইল', detail: `${context?.providers?.length ?? 0}টি প্রতিষ্ঠান যুক্ত`, path: '#' }, { icon: Stethoscope, title: 'ডাক্তার ব্যবস্থাপনা', detail: 'ডাক্তার invitation ও schedule', path: '#' }]
        : role === 'ambulance'
          ? [{ icon: Ambulance, title: 'অ্যাম্বুলেন্স তালিকা', detail: 'সেবা ও availability পরিচালনা করুন', path: '#' }, { icon: Building2, title: 'হাসপাতাল সংযোগ', detail: 'Link request পরিচালনা করুন', path: '#' }]
