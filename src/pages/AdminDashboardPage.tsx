...
export default function AdminDashboardPage() {
  ...
  const [userUpazilaId, setUserUpazilaId] = useState<number | null>(null);
  const [userSpecialtyId, setUserSpecialtyId] = useState<number | null>(null);
  const [userDistricts, setUserDistricts] = useState<District[]>([]);
  const [userUpazilas, setUserUpazilas] = useState<Upazila[]>([]);
  ...
  async function loadUsers(reset = true) {
    ...
    try {
      const userData = await getAdminUserDirectory({
        ...
        medicalType: medicalType === 'all' ? null : medicalType,
        districtId: userDistrictId,
        upazilaId: userUpazilaId,
  ...
  useEffect(() => {
    if (!account || !['admin', 'super_admin'].includes(account.role)) return;
    void Promise.all([getDistricts(), getSpecialties()])
      .then(([districtRows, specialtyRows]) => { setUserDistricts(districtRows); setUserSpecialties(specialtyRows); })
      .catch(() => { setUserDistricts([]); setUserSpecialties([]); });
  }, [account?.user_id, account?.role]);
  useEffect(() => {
    if (!userDistrictId) { setUserUpazilas([]); setUserUpazilaId(null); return; }
    void getUpazilas(userDistrictId).then(setUserUpazilas).catch(() => setUserUpazilas([]));
  }, [userDistrictId]);
  useEffect(() => {
